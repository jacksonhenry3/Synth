...
for (var i = Object.keys(notes).length - 1; i >= 0; i--) {
	...
	if (["G" , "D" , "B"].indexOf(Object.keys(notes)[i].slice(0,-1)) != -1)
	{
...
// for (var i = data.dataCollection.length - 1; i >= 0; i--) {
// 	value = data.dataCollection[i].value
// 	n = scale(value)
// 	osc.frequency.value = n
// };
window.i = 0
function nextNote () {
	value = data.dataCollection[window.i].value
	n = scale(value)
	osc.frequency.value = n
	window.i+=1
	// console.log(value)
